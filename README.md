...
This repository contains the Python scripts for simulating the decentralized optimization of interconnected energy users. The simulation models the interaction between multiple energy users and a Distribution System Operator (DSO) to achieve optimal energy management in a power network. The system uses an iterative optimization process, ensuring privacy through Random Masking Matrix Encryption (RMME) and homomorphic encryption of exchanged data.

The core logic is divided into the following files:

- **main.py**: The main script to run the entire simulation. It initializes the DSO and all energy users, manages the iterative optimization process, and handles information exchange.

- **dso_lib.py**: Defines the DistributionSystemOperator (DSO) class, which manages the power grid topology (represented as a graph) and facilitates secure data exchange between energy users, implementing the core logic of the Grid-Operated Topology-Obfuscating Anonymous Pairing Layer.

- **des_lib.py**: In the single-PC simulation of the co-designed hardware-software architecture, it defines the DistributedEnergySystem class which consolidates Hardware-Enforced Encrypted Coordination Interface functions (local optimization with device constraints/RMME encryption/decryption) and  verifiable encrypted computing layer operations (solving RMME-encrypted subproblems), while maintaining cryptographic isolation through CKKS key management and RMME to preserve dual-privacy.
...
To start the simulation of the iteration process, run the `main.py` script from your terminal to simulate the iterative process of a small interconnected distributed system with 6 energy users (data is stored in the `data/6-bus` directory):
...
Initialize the SimulationSystem with the network configuration (data/6-bus/network_config.json) and configurations for the 6 energy users (data/6-bus/des{i}_config.json).

Set up the cryptographic keys for secure communication between the energy users.

Begin the iterative optimization process, where each energy user solves its local cost minimization problem and securely exchanges boundary information with its neighbors through the DSO.
...
4. System Configuration Upload: Configure the presence of each type of device for each energy user in the `des_config.csv` file, using `0` to indicate absence and `1` to indicate presence.
5. Generate Configuration Files:
   * Run the `create_json.py` script to automatically generate configuration files for each energy user;
